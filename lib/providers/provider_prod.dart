...
class ProviderProd with ChangeNotifier {
  ...
  Future getItemsFirebase([bool filterByUser=false]) async {
    final url=filterByUser==true?Uri.parse(
          'https://first-c92f8-default-rtdb.firebaseio.com/product.json?auth=$token&orderBy="creatorId"&equalTo="$userId"'):Uri.parse(
    ...
    try {
      ...
      mp.forEach((id, data) {
        temp.add(Product(
          ...
          imageUrl: data['imageUrl']?? 'https://images.unsplash.com/photo-1499951360447-b19be8fe80f5?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
